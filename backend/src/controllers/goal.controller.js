...
export const addMoneyToGoal = asyncHandler(async (req, res) => {
  ...
  const updatedGoal = await Goal.findByIdAndUpdate(
    goalId,
    {
      $inc: { currentAmount: currentAmount }
    },
    {
      new: true,
      session,
  ...
  if (updateGoal.currentAmount > updateGoal.finalAmount) {
    await session.abortTransaction();
    session.endSession();
    throw new ApiError(
      400,
  ...
  const updatedUser = await User.findByIdAndUpdate(
    user._id,
    {
      $inc: { currentBalance: -currentAmount }
    },
    {
      new: true,
      session
...
export const deleteGoal = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { user }, "Goal deleted successfully")
    )
});